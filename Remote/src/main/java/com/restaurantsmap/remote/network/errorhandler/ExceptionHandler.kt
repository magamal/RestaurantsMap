    ...
    companion object {
        ...
        fun adapt(throwable: Throwable): Throwable {
            ...
            if (throwable is UnknownHostException || throwable is SocketException || throwable is SocketTimeoutException) {
                return AppException(AppException.ExceptionType.NETWORK, throwable, null)
            } else if (throwable is JsonSyntaxException)
                return AppException(AppException.ExceptionType.PARSING, throwable, null)
            else if (throwable is HttpException)
                return getHttpExceptionType(throwable)
            return AppException(AppException.ExceptionType.UNKNOWN, throwable, null)
        }

        private fun getHttpExceptionType(exception: HttpException): Exception {
            if (exception.code() == UNAUTHORIZED_CODE) {
                return AppException(AppException.ExceptionType.UNAUTHORIZED, exception, null)
            }

            return AppException(AppException.ExceptionType.UNKNOWN, exception, null)
